...
| `PLATFORM_ADMIN_API_KEY` | unset | Server-to-server credential for the platform provisioning endpoints (`POST /api/v1/superuser/organizations` and `POST /api/v1/superuser/organizations/{id}/api-keys`), presented as the `X-Platform-Admin-Key` header. Unset — or shorter than 32 characters — leaves both endpoints returning 503, so an install that never provisions from outside is closed by default. This is how an invite-only install creates tenants with `ENABLE_SIGNUP=false` |
...
<Warning>
`PLATFORM_ADMIN_API_KEY` creates organizations and mints API keys for any tenant. Treat it like a root credential: generate it with `openssl rand -hex 32`, give it only to the provisioning system, and never reuse an organization API key or a super-admin password for it. Organization API keys are rejected on those endpoints outright.
</Warning>
